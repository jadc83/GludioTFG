Vagrant.configure("2") do |config|
	...
	config.vm.network "public_network", ip: "192.168.111.201", netmask: "255.255.240.0"
	...
	config.vm.provider "virtualbox" do |vb|
		vb.memory = 4096
		vb.cpus = 4
		vb.name = "HotelGludio_Final_Solucionado"
	end
	...
	config.vm.provision "shell", env: {
		...
		echo ">>> 5. Configuracion del servidor FTP..."
...
EOF
		...
		echo ">>> 12. Creacion del Servicio Reverb (Apagado por ahora)..."
...
SERVICE
		...
		echo "-------------------------------------------------------------------"
		echo ">>> VAGRANT UP FINALIZADO"
		echo ">>> Tareas pendientes:"
		echo ">>> 1. vagrant ssh"
		echo ">>> 2. cd /var/www/hotelgludio"
		echo ">>> 3. Configurar .env"
		echo ">>> 4. composer install && npm install && npm run build"
		echo ">>> 5. sudo systemctl enable --now reverb.service"
		echo "-------------------------------------------------------------------"
	SHELL
end