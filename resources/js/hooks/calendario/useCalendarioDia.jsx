...
export default function useCalendarioDia(mapaPrecios, formatearISO) {
    const componentesDia = useMemo(() => ({
        Day: ({ date, disabled, ...props }) => {
                ...
                elementosAdicionales.push(
                    <div
                        ...
                        style={{
                            ...
                            left: 0,
                            height: '6px',
                            backgroundColor: ocupacion > 80 ? '#7a0202' : ocupacion > 50 ? '#6b7280' : '#d1d5db',
                            width: `${ocupacion}%`,
            ...
            if (React.isValidElement(contenido) && elementosAdicionales.length > 0) {
                const hijosOriginales = contenido.props.children;
                contenido = React.cloneElement(contenido, {}, [hijosOriginales, ...elementosAdicionales]);
            }
