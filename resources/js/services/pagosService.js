...
async function crearCheckoutSession(reservaId, payload) {
    try {
        ...
        return res ?? { success: false, error: 'Empty response from crearCheckoutSession' };
    } catch (err) {
...
async function checkSession(sessionId) {
    try {
        ...
        return res ?? { success: false, error: 'Empty response from checkSession' };
    } catch (err) {
        return { success: false, error: err?.response?.data?.error || err?.message || String(err) };
    }
}
...
async function confirmar(paymentIntentId, pagoId = null) {
    try {
        ...
        return res ?? { success: false, error: 'Empty response from confirmar' };
    } catch (err) {
        return { success: false, error: err?.response?.data?.error || err?.message || String(err) };
    }
}
...
async function crearPaymentIntent(reservaId, monto) {
    try {
        ...
        return res ?? { success: false, error: 'Empty response from crearPaymentIntent' };
    } catch (err) {
        return { success: false, error: err?.response?.data?.error || err?.message || String(err) };
    }
}
...
async function crearPaymentIntentStandalone(monto, opts = {}) {
    try {
        ...
        return res ?? { success: false, error: 'Empty response from crearPaymentIntentStandalone' };
    } catch (err) {
        return { success: false, error: err?.response?.data?.error || err?.message || String(err) };
    }
}
...
export { crearCheckoutSession, checkSession, confirmar, crearPaymentIntent, crearPaymentIntentStandalone };
