            ...
            <div className={`drawer-panel ${abierto ? 'abierto' : 'cerrado'}`}>
                ...
                {cliente && (
                    <form onSubmit={enviar} className="form-cliente">
                        ...
                        <Campo id="direccion" label="Dirección" as="textarea" rows={3} value={form.direccion || ''} onChange={cambiar} error={errores.direccion} />

                        <PrimaryButton type="submit" className="w-full mt-4">
                            {guardando ? 'Actualizando...' : 'Actualizar Cliente'}
                        </PrimaryButton>
                    </form>
                )}
            </div>
