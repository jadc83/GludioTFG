...
export default function CreateHabitacion() {
  ...
  const { form, cambiar, errores, guardando, capacidadFija, fotos, previews,
          agregarFotos, quitarFoto, enviar, MAX_FOTOS } = useCreateHabitacion(
    modoContinuo ? undefined : () => setAbierto(false)
  );
      ...
      <dialog className={`drawer-modal ${abierto ? 'modal-open' : ''}`}>
        <div className={`drawer-panel ${abierto ? 'abierto' : 'cerrado'}`}>
          <header className="drawer-header">
            ...
            <button onClick={() => setAbierto(false)} className="btn-cerrar">✕</button>
          </header>
          ...
          <form onSubmit={enviar} className="form-habitacion">
            ...
            <div className="form-grid">
              <Campo id="numero" label="Número" type="text" value={form.numero}
                     onChange={cambiar} error={errores.numero} required
                     placeholder="Ej: 101"/>

              <Campo id="tipo" label="Tipo" as="select" value={form.tipo}
                     onChange={cambiar} error={errores.tipo}>
                <option value="doble">Doble</option>
                <option value="suite">Suite</option>
                <option value="familiar">Familiar</option>
              </Campo>

              <Campo id="precio_noche" label="Precio €" type="number" step="0.01"
                     value={form.precio_noche} onChange={cambiar} error={errores.precio_noche} required/>

              <Campo id="capacidad" label="Capacidad" type="number" min="1"
                     value={form.capacidad} onChange={cambiar} error={errores.capacidad} required
                     readOnly={capacidadFija} classNameExtra={capacidadFija ? 'readonly' : ''}/>
            </div>

            <Campo id="estado" label="Estado" as="select" value={form.estado}
                   onChange={cambiar} error={errores.estado}>
            ...
            <InputFotos fotos={fotos} previews={previews} onAgregar={agregarFotos}
                       onQuitar={quitarFoto} error={errores.fotos} maxFotos={MAX_FOTOS}/>

            <Campo id="descripcion" label="Descripción" as="textarea"
                   value={form.descripcion} onChange={cambiar} error={errores.descripcion}
                   placeholder="Detalles públicos..."/>

            <Campo id="notas" label="Notas Privadas" as="textarea" rows={3}
                   value={form.notas} onChange={cambiar} error={errores.notas}
