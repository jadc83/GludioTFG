...
export default function IndexDepartamentos() {
    ...
    const { departamentosPaginados, totalPaginas, inicio, fin } = useMemo(() => {
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-6 px-4 py-6">
            <HeaderPanel titulo="Departamentos" subtitulo="Gestión de Departamentos" />
            ...
            {departamentos.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <h3 className="text-lg font-black uppercase tracking-tight text-gray-900">No hay departamentos</h3>
                    <p className="mt-1 text-sm text-gray-400">Crea departamentos mediante seeders o el panel.</p>
                </div>
            ) : (
                <>
                        ...
                        <table className="responsive-table w-full border-collapse text-left">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {departamentosPaginados.map((d) => (
                                    <tr key={d.id} className="group transition-colors hover:bg-gray-50/50">
                                        <td className="px-6 py-4" data-label="Nombre">
                                            <div className="text-sm font-black uppercase tracking-tight text-gray-900">{d.name}</div>
                                        </td>
                                        <td className="px-6 py-4 text-right" data-label="Acciones">
                                            <div className="flex justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                                                <button onClick={() => abrirDetalle(d)} className="rounded-lg p-2 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600">
                    ...
                    <Paginacion paginaActual={paginaActual} totalPaginas={totalPaginas} inicio={inicio} fin={fin} total={departamentos.length} onCambiarPagina={setPaginaActual} etiqueta="Departamentos" />
                </>
            )}
