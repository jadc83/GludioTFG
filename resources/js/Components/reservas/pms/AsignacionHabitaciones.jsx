...
export default function AsignacionHabitaciones({
    ...
    return (
        <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-gray-100 bg-[#7a0202] px-6 py-5">
                <div>
                    <h3 className="text-xs font-bold uppercase tracking-widest text-white">
                        Asignación de habitaciones
                    </h3>
                    <p className="mt-1 text-[10px] text-white/90">
                        Selecciona habitaciones disponibles para asignar. Usa el
                        botón ✕ para desasignar.
                    </p>
                </div>
                <div>
                    {isCheckedOut ? (
                        <div className="rounded-md bg-yellow-50 px-4 py-2 text-xs font-medium text-yellow-800">
                            Asignación deshabilitada: reserva finalizada
                        </div>
                    ) : (
                        <button
                            onClick={onGuardar}
                    ...
                    >
                        <div className="flex items-center gap-3 border-b border-gray-100 pb-3">
                            <div className="flex-1">
                                ...
                                {hSlot.precio &&
                                    (() => {
                                        ...
                                        const parseDate = (s) =>
                                            s ? new Date(s) : null;
                                        const slotCheckIn = parseDate(
                                            hSlot.check_in,
                                        );
                                        const slotCheckOut = parseDate(
                                            hSlot.check_out,
                                        );
                                        const noches =
                                            slotCheckIn && slotCheckOut
                                                ? Math.max(
                                                      0,
                                                      Math.ceil(
                                                          (slotCheckOut -
                                                ...
                                                : null;
                                        const precioNoche =
                                            hSlot.precio_noche != null
                                                ? Number(hSlot.precio_noche)
                                                : noches && noches > 0
                                                  ? Number(hSlot.precio) /
                                                    noches
                                                  : Number(hSlot.precio);
                            ...
                            {hSlot.habitacion_id && (
                                <button
                                    onClick={() =>
                                        onDesasignar(hSlot.habitacion_id)
                                    }
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 md:grid-cols-6">
                            <button
                                ...
                                disabled={isCheckedOut}
                                className={`rounded-xl border-2 p-3 text-center transition ${!habitacionesSeleccionadas[idx] ? 'border-[#7a0202] bg-[#7a0202]/10 text-[#7a0202]' : 'border-gray-100 text-gray-400 hover:border-gray-200'} ${isCheckedOut ? 'cursor-not-allowed opacity-50' : ''}`}
                            >
                            ...
                            {(() => {
                                const filtered = habitacionesDisponibles.filter(
                                    (habFisica) =>
                                        !hSlot.tipo ||
                                        (habFisica.tipo &&
                                            habFisica.tipo.toLowerCase() ===
                                                hSlot.tipo.toLowerCase()),
                                );
                                ...
                                return filtered.map((habFisica) => {
                                    const isSelected =
                                        habitacionesSeleccionadas[idx] ===
                                        habFisica.id;
                                    const isUsedElsewhere =
                                        habitacionesSeleccionadas.some(
                                            (id, i) =>
                                                id === habFisica.id &&
                                                i !== idx,
                                        );
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative rounded-xl border-2 p-3 transition ${isSelected ? 'border-[#7a0202] bg-[#7a0202]/10 text-[#7a0202]' : isUsedElsewhere ? 'cursor-not-allowed opacity-20 grayscale' : 'border-gray-100 hover:border-gray-300'} ${isCheckedOut ? 'cursor-not-allowed opacity-50' : ''}`}
                                        >
