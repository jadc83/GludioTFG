...
export default function OpcionesPago({
	...
	const reservaData = typeof prepararDatosReserva === 'function'
		? prepararDatosReserva()
		: null;

	const esReservaInvalida = !reservaData ||
		!reservaData.check_in ||
		!reservaData.check_out ||
		!Array.isArray(reservaData.habitaciones) ||
		reservaData.habitaciones.length === 0;

	const cardBaseClass = 'relative flex flex-col p-3 border-2 transition-all duration-300 cursor-pointer rounded-xl group';
	...
	return (
		...
		<div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
							...
							<div className="rounded-xl bg-gris p-1">
								{esReservaInvalida ? (
									<div className="rounded-xl border border-yellow-300 bg-yellow-50 p-3 text-center">
										<p className="mb-3 text-[11px] font-bold text-yellow-800 uppercase">
											Faltan fechas o habitaciones
										</p>
										<p className="mb-4 text-[10px] text-yellow-700">
											Selecciona fechas y al menos una habitación antes de continuar.
										</p>
										<button
											type="button"
											onClick={() => typeof setPasoActual === 'function' && setPasoActual(1)}
											className="rounded bg-[#7a0202] px-4 py-2 text-[10px] font-bold text-white uppercase"
										>
											Seleccionar fechas
										</button>
									</div>
									...
									<ErrorBoundary>
										<FormularioPago
											...
											onPagoExitoso={(data) => {
												...
												if (loc) window.location.href = `/reservas/${loc}/edit`;
											}}
							...
							<div className="space-y-3">
								<div className="flex flex-col items-center rounded-xl border border-dashed border-gray-200 bg-gray-50 p-3 text-center">
									...
									<p className="mt-2 text-[11px] font-bold text-gray-600">
										No se realizará ningún cargo ahora. <br /> La confirmación es inmediata.
									</p>
								</div>
								...
								<div className="flex items-center justify-end">
									<button
										...
										className="w-full sm:w-1/2 md:w-1/3 rounded-xl bg-[#7a0202] py-4 text-[12px] font-black uppercase tracking-[0.3em] text-white shadow-xl transition-all hover:bg-black disabled:opacity-20 disabled:grayscale active:scale-[0.97]"
									>
			...
			<div className="mt-3 flex flex-col gap-1 border-t border-gray-100 pt-2">
					...
					{cuponValido && (
				<div className="flex items-center gap-2 rounded-md bg-green-50 px-3 py-1 text-xs font-bold text-green-700 border border-green-100 shadow-sm">
							<span>Cupón aplicado: Ahorras €{cuponValido.descuento.toFixed(2)}</span>
						</div>
					)}
					...
					<label className="mt-1 flex items-start gap-2 cursor-pointer">
						<input
							...
							className="rounded border-gray-300 text-[#7a0202] focus:ring-[#7a0202] mt-0.5"
						/>
