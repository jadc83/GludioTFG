		...
		<div className="paso2-habitaciones relative flex flex-col h-screen md:h-auto max-h-[100dvh] md:max-h-none mx-auto w-full max-w-5xl bg-white md:rounded-xl md:border md:border-gray-200 md:shadow-lg overflow-hidden">
			<header className="flex-none px-4 py-3 sm:px-6 sm:py-5 border-b bg-white z-30">
				<div className="flex items-center justify-between">
					<div>
						<h1 className="text-base sm:text-lg font-extrabold tracking-tight text-gray-900 leading-tight">
			...
			<div className="md:hidden flex-none px-4 py-2.5 bg-gray-50 border-b flex justify-between items-center">
				<div className="flex flex-col">
					<span className="text-[8px] font-black text-gray-400 uppercase leading-none mb-1">Régimen</span>
					<span className="text-[10px] font-bold text-gray-700 truncate max-w-[150px]">
						{tarifas.find(t => t.id === selectedTarifas.id)?.nombre || 'Estándar'}
					</span>
				</div>
				<button
				...
				>
					...
					{mostrarTarifasMobile ? 'Cerrar' : 'Ajustar'}
				</button>
			</div>
			...
			<main className="flex flex-col md:flex-row flex-1 overflow-hidden relative">
					...
					{mostrarTarifasMobile && (
						<div className="md:hidden sticky top-0 mb-6 p-5 rounded-2xl bg-white border-2 border-[#7a0202] shadow-2xl z-40 animate-in fade-in zoom-in-95 duration-200">
							<div className="flex justify-between items-center mb-4">
								<h3 className="text-[11px] font-black uppercase text-gray-900">Opciones de estancia</h3>
								<button onClick={() => setMostrarTarifasMobile(false)} className="text-[10px] font-bold text-gray-400 uppercase">Cerrar</button>
							</div>
							<TarifasSelector
								tarifas={tarifas}
						...
						{estaCargandoHabitaciones ? (
							<div className="py-24 flex flex-col items-center justify-center">
								...
								<p className="mt-4 text-xs text-gray-400 font-bold uppercase tracking-widest text-center">
									Estamos buscando<br/>disponibilidad para ti...
								</p>
							</div>
						) : (
				...
				<aside className="hidden md:flex w-80 border-l border-gray-100 bg-white flex-col">
					...
					<div className="p-5 bg-white border-t border-gray-100">
						<Boton
							variant="primary"
							className="w-full bg-[#7a0202] hover:bg-black py-4 text-[11px] font-black tracking-[0.2em] uppercase"
							onClick={avanzarPaso}
							disabled={totalSeleccionado === 0}
						>
							Continuar ({totalSeleccionado})
						</Boton>
						<button
							onClick={retrocederPaso}
							className="w-full mt-4 text-[9px] font-black text-gray-400 uppercase hover:text-[#7a0202] transition-colors"
						>
							← Cambiar fechas
						</button>
					</div>
			...
			<div className="md:hidden relative z-50">
				<MobileStickyBar>
					<div className="max-w-5xl mx-auto flex items-center justify-between gap-4 px-3 py-1">
						<div className="flex flex-col">
							<span className="text-[9px] uppercase font-black text-gray-400 leading-none mb-1">Subtotal</span>
						...
						<div className="flex items-center gap-2">
							<button
								onClick={retrocederPaso}
								className="h-12 w-12 flex items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-500 shadow-sm active:bg-gray-50 active:scale-95 transition-all"
								aria-label="Volver"
							>
								<ArrowLeftIcon className="h-5 w-5" />
							</button>
							<Boton
								className={`h-12 px-6 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-lg ${
									totalSeleccionado > 0
									? 'bg-[#7a0202] text-white shadow-red-900/30'
									: 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'
								}`}
								onClick={avanzarPaso}
								disabled={totalSeleccionado === 0}
							>
								Siguiente {totalSeleccionado > 0 && `(${totalSeleccionado})`}
							</Boton>
						</div>
					</div>
				</MobileStickyBar>
