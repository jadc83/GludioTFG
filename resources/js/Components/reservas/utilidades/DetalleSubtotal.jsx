...
export default function DetalleSubtotal({ habitacionesSeleccionadas = {}, rango = {}, tarifasSeleccionadas = {}, tarifas = [], tipos = {}, preciosPorTipo = {}, soloSubtotal = false }) {
	...
	const subtotalHabitaciones = Object.entries(habitacionesSeleccionadas || {}).reduce((acc, [tipo, h]) => {
		...
		const precioDesdeTipos = Number(precioCalculado ?? tipoInfo.precioEntreNoche ?? tipoInfo.precioNoche ?? tipoInfo.precioTipo ?? tipoInfo.precioMinimo ?? 0);
		const precioPorNoche = precioDesdeSeleccion > 0 ? precioDesdeSeleccion : precioDesdeTipos;
	...
	const habitacionesTotalSeleccionadas = Object.values(habitacionesSeleccionadas || {}).reduce((s, v) => s + (Number(v.cantidad || 0)), 0);
	let cargoTarifas = 0;
	...
	if (tarifas && Object.keys(tarifasSeleccionadas || {}).length > 0) {
		tarifas.forEach(t => {
			...
			if (slug.includes('desayuno')) {
				cargo = 0;
			} else if (slug.includes('media') || slug.includes('pension') || slug.includes('media-pension')) {
				cargo = Number(mod) * Number(numeroNoches) * Math.max(0, habitacionesTotalSeleccionadas);
			} else {
				cargo = Number(mod);
			}
	...
	if (soloSubtotal) {
		return (
			<div className="text-right">
				<div className="text-lg font-extrabold text-[#7a0202]">{formatearMoneda(subtotal)}</div>
			</div>
		);
	}
	...
	return (
		<div className="w-full bg-gris rounded p-2">
			<div className="flex items-center justify-between text-[12px]">
				...
				<span className="text-lg font-extrabold text-[#7a0202]">{formatearMoneda(subtotal)}</span>
			</div>
