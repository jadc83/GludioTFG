...
export default function ProfileDashboard({
    ...
    const allowedProfileContainer =
        empleado ||
        ['admin', 'encargado', 'operario', 'auxiliar'].some((r) =>
            roles.includes(r),
        );
    const middleCols = allowedProfileContainer
    ...
    const fetchSummary = useCallback(async () => {
        ...
        try {
            ...
            if (tRes.ok) {
                ...
                const enriched = (data.turnos || []).map((t) => ({
                    ...t,
                ...
                const inWeek = enriched
                    ...
                    .sort(
                        (a, b) =>
                            (a.parsedStart ? a.parsedStart.getTime() : 0) -
                            (b.parsedStart ? b.parsedStart.getTime() : 0),
                    );
                ...
                if (inWeek.length > 0) {
                    ...
                    turnos = enriched
                        .filter(
                            (t) =>
                                (t.parsedEnd && t.parsedEnd >= now) ||
                                (t.parsedStart && t.parsedStart >= now),
                        )
                        .sort(
                            (a, b) =>
                                (a.parsedStart
                ...
                const groupByDay = (arr) => {
                    ...
                    const days = Object.values(map).sort(
                        (a, b) => a.date - b.date,
                    );
                    days.forEach((day) =>
                        day.items.sort(
                ...
                console.debug('ProfileDashboard turnos fetched', {
                    total: data.turnos?.length || 0,
            ...
            // completed tareas
            const cRes = await fetch('/api/tareas/completed', {
                credentials: 'same-origin',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    Accept: 'application/json',
            ...
            // active tareas count
            const aRes = await fetch('/api/tareas', {
                credentials: 'same-origin',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    Accept: 'application/json',
    ...
    return (
        <main aria-label="Panel de usuario">
            {/* existing grid */}
                                    ...
                                    <>
                                        ...
                                        <div className="mt-4">
                                            <EmpleadoProfile
                                                ...
                                                showAssignState={false}
                                            />
                                        </div>
                                    </>
                                ) : (
                                    <div className="mt-4 text-sm text-gray-500">
                                        No tienes acceso a tareas o turnos.
                                    </div>
                                )
                            ) : null}
                        </div>
