...
export default function EmpleadoProfile({ habitaciones = [], showAssignState = true }) {
    ...
    const fetchMyTareas = async () => {
        try {
            ...
            if (!res.ok) { setHasActiveTask(false); setActiveTareas([]); return; }
            const data = await res.json();
            const tareas = Array.isArray(data.tareas) ? data.tareas : [];
            setActiveTareas(tareas);
            setHasActiveTask(tareas.length > 0);
        } catch (e) { console.error('fetch tareas failed', e); setHasActiveTask(false); setActiveTareas([]); }
    };

    const completeTarea = async (tareaId) => {
        try {
            const csrf = getCsrf();
            const res = await fetch(`/api/tareas/${tareaId}/complete`, { method: 'POST', credentials: 'same-origin', headers: { 'X-CSRF-TOKEN': csrf, 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } });
            if (!res.ok) {
                const err = await res.json().catch(() => ({}));
                window.dispatchEvent(new CustomEvent('app-toast', { detail: { message: err.error || 'Error al completar tarea', type: 'error' } }));
                return;
            }
        ...
        } catch (e) { console.error(e); window.dispatchEvent(new CustomEvent('app-toast', { detail: { message: 'Error al completar tarea', type: 'error' } })); }
    };

    const cancelTarea = async (tareaId) => {
        try {
            const csrf = getCsrf();
            const res = await fetch(`/api/tareas/${tareaId}/cancel`, { method: 'POST', credentials: 'same-origin', headers: { 'X-CSRF-TOKEN': csrf, 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } });
            if (!res.ok) {
                const err = await res.json().catch(() => ({}));
                window.dispatchEvent(new CustomEvent('app-toast', { detail: { message: err.error || 'Error al desasignar tarea', type: 'error' } }));
                return;
            }
            window.dispatchEvent(new CustomEvent('app-toast', { detail: { message: 'Tarea desasignada', type: 'success' } }));
            window.dispatchEvent(new Event('tareas:updated'));
        } catch (e) { console.error(e); window.dispatchEvent(new CustomEvent('app-toast', { detail: { message: 'Error al desasignar tarea', type: 'error' } })); }
    };
                ...
                {loading ? (
                    ...
                    <div>
                        ...
                        {(() => {
                            ...
                            const unassignedRooms = rooms.filter(r => !assignedIds.includes(r.id));
                            if (unassignedRooms.length === 0) {
                                return <div className="p-6 text-sm text-gray-500">No hay habitaciones en limpieza.</div>;
                            }
                            return (
                                                ...
                                                <button
                                                    ...
                                                    onClick={async () => {
                                                        ...
                                                        try {
                                                            ...
                                                            if (!res.ok) {
                                                                if (res.status === 419) { try { window.alert('Sesión expirada o token inválido. Se recargará la página para renovarla.'); } catch (e) {} window.location.reload(); return; }
                                                                const err = await res.json().catch(() => ({}));
                        ...
                        {/* MIS TAREAS ACTIVAS */}
                        <div className="mt-6">
                            <h5 className="font-black uppercase text-xs text-gray-700 mb-3">Mis tareas activas</h5>
                            {activeTareas && activeTareas.length > 0 ? (
                                <div className="space-y-3">
                                    {activeTareas.map(t => (
                                        <div key={t.id} className="rounded-lg border border-gray-100 p-3 bg-gray-50 flex items-center justify-between">
                                            <div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button className="text-xs bg-rose-500 text-white px-3 py-1 rounded" onClick={() => completeTarea(t.id)}>Completar</button>
                                                <button className="text-xs border border-gray-200 text-gray-700 px-3 py-1 rounded" onClick={() => cancelTarea(t.id)}>Desasignar</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
