...
export default function ReservationRow({ reserva, configEstado }) {
    return (
        <tr className="group transition-colors hover:bg-gray-50/50">
            ...
            <td className="py-6 text-center text-xs font-black text-gray-900" data-label="Noches">{reserva.noches}</td>

            <td className="py-6 text-sm font-black italic text-gray-900" data-label="Inversión">{reserva.monto_total}</td>

            <td className="py-6" data-label="Estado">
                <span className={`inline-flex items-center rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-widest shadow-sm ${configEstado[reserva.estado] || configEstado.default}`}>
                    {reserva.estado}
                </span>
            </td>

            <td className="py-6 text-right" data-label="">
                <Link href={`/reservas/${reserva.id}`} className="inline-flex items-center gap-1.5 rounded-xl bg-gray-50 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-gray-400 transition-all hover:text-[#7a0202]">
                    Detalles
