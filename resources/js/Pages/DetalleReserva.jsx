                ...
                <div className="mx-auto max-w-5xl px-4">
                    <div className="space-y-3">
                        ...
                        <div className="relative rounded-lg bg-white p-3 shadow-md flex items-center justify-between">
                            ...
                            {String(reserva.status || '').toLowerCase().includes('cancel') && (
                                <div className="absolute top-3 right-3 bg-[#7a0202] text-white px-3 py-1 text-sm font-bold rounded shadow-lg">CANCELADA</div>
                            )}
                        </div>

                        {/* Resumen (total y acciones) movido arriba */}
                                ...
                                {reserva.pago === 'pagado' && (
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            if (!confirm('Solicitar reembolso para esta reserva?')) return;
                                            setIsProcessing(true);
                                            import('axios').then(({ default: axios }) => {
                                                axios.post(`/reservas/${reserva.id}/reembolsar`)
                                                    .then((res) => {
                                                        showToast(res?.data?.message || 'Reembolso solicitado correctamente.', 'success');
                                                        return axios.get(`/reservas/buscar/${reserva.localizador}`);
                                                    })
                                                    .then((res2) => {
                                                        if (res2?.data?.reserva) {
                                                            setReserva(prev => ({ ...prev, ...res2.data.reserva }));
                                                        }
                                                    })
                                                    .catch((err) => {
                                        ...
                                        className={`bg-white text-[#7a0202] font-semibold px-3 py-2 rounded shadow-sm hover:opacity-90 text-sm ${isProcessing ? 'opacity-60 cursor-wait' : ''}`}
                                    >
                                        {isProcessing ? 'Procesando…' : 'Pedir reembolso'}
                                    </button>
                                )}
                            </div>
                        </div>

                        {/* Agrupado: Info principal, habitaciones, hotel, horarios, políticas y servicios en un único contenedor */}
                        ...
                        {!mostrarExtender && !String(reserva.status || '').toLowerCase().includes('cancel') && (
                            <button
